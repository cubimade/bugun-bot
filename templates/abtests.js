...
async function loadTests() {
  try {
    ...
    TESTS = r.tests || [];
    renderTests();
  } catch (e) { $("testsList").innerHTML = emptyState("⚠️", "Yuklashda xatolik: " + e.message); }
}
function pct(part, total) { return total ? Math.round((part / total) * 100) : 0; }
function renderTests() {
  if (!TESTS.length) { $("testsList").innerHTML = emptyState("🧪", "Hali test yo'q — birinchisini yuqorida yarating"); return; }
  $("testsList").innerHTML = TESTS.map(function (t) {
    ...
    if (t.status === "running" || t.status === "stopped") {
      if (n < 20) verdict = '<span class="small muted">Hali ma\\'lumot kam (' + n + ' kontakt) — xulosa uchun kamida 20 kerak</span>';
        ...
        verdict = lead
          ? '<span class="small">' + (diff >= 10 ? "📊 <strong>" + lead + " varianti sezilarli yaxshi</strong> (+" + diff + " f.p.)" : "📊 " + lead + " biroz oldinda (+" + diff + " f.p.) — farq hali sezilarli emas") + "</span>"
    ...
    const varRow = function (v, data, rate, conv, text) {
      return '<div style="background:var(--panel2);border-radius:11px;padding:10px 13px">' +
        ...
        (t.status !== "finished" ? '<span style="flex:1"></span><button class="btn btn-sm" onclick="finishTest(' + t.id + ',\\'' + v + '\\')">🏆 G\\'olib qilish</button>' : "") +
        "</div>" +
    ...
    return '<div class="card" style="margin-bottom:12px;' + (t.status === "finished" ? "opacity:.65" : "") + '">' +
      '<div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:10px">' +
        ...
        '<span class="badge b-indigo">' + (TYPE_LBL[t.test_type] || t.test_type) + "</span>" +
        '<span class="badge ' + (t.status === "running" ? "b-green" : t.status === "finished" ? "b-gray" : "b-amber") + '">' +
          (t.status === "running" ? "▶️ ketmoqda" : t.status === "finished" ? "🏁 yakunlangan" : "⏸ to'xtatilgan") + "</span>" +
        '<span class="small muted">A ' + t.split_percent + "% / B " + (100 - t.split_percent) + "%</span>" +
        '<span style="flex:1"></span>' +
        (t.status === "running" ? '<button class="btn btn-sm" onclick="setTestStatus(' + t.id + ',\\'stopped\\')">⏸ To\\'xtatish</button>' : "") +
        (t.status === "stopped" ? '<button class="btn btn-sm" onclick="setTestStatus(' + t.id + ',\\'running\\')">▶️ Davom ettirish</button>' : "") +
      "</div>" +
