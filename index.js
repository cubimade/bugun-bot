...
async function cachedAnalytics(key, fn) {
...
APP.get("/api/export/contacts.csv", protect, async (req, res, next) => {
  if (!requireDb(req, res)) return;
  try {
    const period = normalizePeriod(req.query.period);
...
APP.get("/api/export/report.csv", protect, async (req, res, next) => {
  if (!requireDb(req, res)) return;
  try {
    const period = normalizePeriod(req.query.period);
