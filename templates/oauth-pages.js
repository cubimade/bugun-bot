...
// ============================================================
import { esc, APP_VERSION } from "./components.js";
...
export function renderOAuthSuccessPage(profile, subscribed) {
  ...
  const avatar = profile.picture
    ...
    : `<div class="stat-ic" style="width:76px;height:76px;border-radius:50%;font-size:32px;background:linear-gradient(135deg,#833AB4,#FD1D1D,#FCAF45)">📸</div>`;
  ...
  const warn = subscribed
    ...
    : `<div class="card" style="padding:11px 13px;margin-top:16px;text-align:left;border-color:rgba(251,191,36,.5);background:rgba(251,191,36,.08)">
         <strong class="small">⚠️ Webhook obunasi avtomatik yoqilmadi</strong>
  ...
  return shell({
    ...
    <div style="display:flex;gap:9px;justify-content:center;margin-top:20px;flex-wrap:wrap">
      ...
      <a class="btn" href="/dashboard/knowledge">🧠 Bilim bazasi</a>
    </div>
...
export function renderOAuthErrorPage(title, message, hint = "") {
  return shell({
    ...
    <div style="display:flex;gap:9px;justify-content:center;margin-top:18px;flex-wrap:wrap">
      <a class="btn btn-primary" href="/dashboard/connect/instagram">↻ Qayta urinish</a>
      <a class="btn" href="/dashboard/accounts">Akkauntlar</a>
    </div>`,
  });
}
