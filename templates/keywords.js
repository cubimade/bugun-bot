...
export function renderKeywordsPage() {
  ...
  <div class="card" style="margin-bottom:16px">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px" class="kw-cols">
      ...
      <div><label class="lbl">Rasm URL (ixtiyoriy, https://...)</label>
        <div style="display:flex;gap:8px">
          ...
          <button class="btn" type="button" onclick="openMediaPicker()" data-tip="Media kutubxonasidan tanlash">${ICONS.media}</button>
        </div>
    ...
    <details style="margin-bottom:12px">
      <summary style="cursor:pointer;font-size:14px;padding:6px 0;display:flex;align-items:center;gap:6px">${ICONS.settings} Qo'shimcha sozlamalar</summary>
  ...
  <div class="card" style="margin-bottom:16px">
    <h3 style="margin-bottom:4px;display:flex;align-items:center;gap:8px">${ICONS.flask} Sinab ko'rish</h3>
...
function renderRules() {
  ...
  $("rulesList").innerHTML = '<div class="group-list">' + RULES.map((r, i) => {
    const chips = [MATCH_LABELS[r.match_type] || "ichida bo'lsa", esc(r.project_name || "Barcha akkauntlar")];
    if ((r.media_urls || []).length || r.media_url) chips.push(((r.media_urls || []).length || 1) + " ta rasm");
    if ((r.buttons || []).length) chips.push(r.buttons.length + " tugma");
    if (r.delay_sec) chips.push(r.delay_sec + "s kechikish");
    if (r.once_per_contact) chips.push("bir marta");
    if (r.work_hours_only) chips.push("ish vaqti");
    return \`
    <div class="group-row" style="align-items:flex-start;\${r.is_active ? "" : "opacity:.55"}">
      <div class="row-body">
        <p class="row-title" style="display:flex;align-items:center;gap:6px">${ICONS.key} \${esc(r.keyword)}</p>
        <p class="row-sub">\${chips.join(" · ")}</p>
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:6px">
          \${r.priority ? \`<span class="pill pill-plain" title="Ustuvorlik — katta raqam ustun">↑ \${r.priority}</span>\` : ""}
          <span class="pill \${r.hit_count ? "pill-ok" : "pill-plain"}" title="Necha marta ishlagan">\${r.hit_count} marta</span>
          <span class="pill pill-plain" title="Qoida javobidan keyin necha kishi yozgan">\${r.reply_count || 0} javob</span>
        </div>
      </div>
      <div style="display:flex;gap:4px;flex-shrink:0;flex-wrap:wrap;justify-content:flex-end">
        <button class="btn btn-plain btn-sm" title="Yuqoriga — ustuvorlik oshadi" onclick="moveRule(\${r.id}, -1)">↑</button>
        <button class="btn btn-plain btn-sm" title="Pastga" onclick="moveRule(\${r.id}, 1)">↓</button>
    ...
    </div>\${i < RULES.length - 1 ? '<div class="separator no-avatar"></div>' : ""}\`;
  }).join("") + '</div>';
}
...
async function openMediaPicker() {
  ...
  try {
    ...
    if (!imgs.length) {
      $("modalBody").innerHTML = emptyState('${ICONS.media}', "Kutubxona bo'sh — Media sahifasidan rasm yuklang",
    ...
    if (!r.matched) {
      ...
      $("kwTestOut").innerHTML = \`
        <div class="card" style="padding:12px;border-color:rgba(52,211,153,.45);background:rgba(52,211,153,.07)">
          <div class="small" style="margin-bottom:8px">${ICONS.check} Ishga tushadi:
            <span class="pill pill-plain">${ICONS.key} \${esc(rl.keyword)}</span>
